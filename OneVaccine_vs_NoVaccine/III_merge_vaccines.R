...
#'
#'
#' FIX: add another coloumn: VACC_DOSES:
#' Count amount of doses for each animal. Static number for each animal
#'
...
# OTHER approach, if looking at vaccines each parity prior to the lac-Phase
# (however not sure, when they vaccinate)

# We need 3 vaccinations. We can therefore just sort by the one,
# taken just around the calving time... 
# We don't need to worry about Kontrol dato

# We need: Vaccination data max 60 days before Calving_date
# (typically 7 weeks)
# Vaccination date max 5 days after Calving_date
# vacc_branch <- left_join(branch, vaccination, by = "DYR_ID", sort="TRUE",allow.cartesian=TRUE)
# vacc_branch_2 <- vacc_branch %>%
#   group_by(DYR_ID, PARITY) %>%
#   filter(VACCINATION_DATE - 5 < CALVING_DATE |is.na(VACCINATION_DATE)) %>%
#   filter(VACCINATION_DATE + 50 > CALVING_DATE |is.na(VACCINATION_DATE)) %>%
#   dplyr::select(DYR_ID, PARITY, VACC, VACCINATION_DATE)
# vaccination <- vacc_branch_2 %>%
#   drop_na()
 



# Join vaccinations to master (df6) ----------------------------------

# df7+8: + vaccination
df7 <- left_join(df6, vaccination, by = c("DYR_ID"), sort="TRUE",allow.cartesian=TRUE)

# just if duplicate check would be needed...
df8 <- df7 %>% 
...
# temp saving
rm(df6, df7, vaccination); gc()
save.image("K:/paper_vaccine/III_merge_vaccine_TEMP3.RData") 


# remove SCC < VACCINATIONS_DATO if any --------------------

df9 <- df8 %>%
  filter(KONTROLDATO - 13  > VACCINATION_DATE |is.na(VACCINATION_DATE)) %>%
  ungroup()

# visualizing data with vaccination to ensure KONTROLDATO > VACCINATION_DATE
#disp_df9 <- df9 %>%
#  filter(VACC == 1) %>%
#  select(DYR_ID, PARITY, KONTROLDATO, CALVING_DATE, VACCINATION_DATE) %>%
#  slice(1:1000)
